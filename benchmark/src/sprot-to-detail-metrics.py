...
def remove_unreliable_gos(acc_to_gos, term_to_freq, inner_to_outers, thres=10):
    ...
    for acc, gos in acc_to_gos.items():
        ret_acc_to_gos[acc] = set([go for go in gos if term_to_freq[go] >= thres])
    ret_inner_to_outers = {}
    for inner, outers in inner_to_outers.items():
        ret_inner_to_outers[inner] = set([outer for outer in outers if outer in ret_acc_to_gos and inner in ret_acc_to_gos and ret_acc_to_gos[inner] and ret_acc_to_gos[outer]])
    return ret_acc_to_gos, ret_inner_to_outers
...
def display_sim22s_selfexc(inner_to_outers, acc_to_gos, term_to_ancestors, term_to_freq, tot_freq):
    perinn_sim22s = []
    perclu_sim22s = []
    for i, (inner, outers) in enumerate(inner_to_outers.items()):
        if len(outers) < 2: continue
        for acc1 in outers:
            for acc2 in outers:
                if acc1 == acc2: continue
                gos1, gos2 = (acc_to_gos[acc1], acc_to_gos[acc2])
                sim22elem = sim22(gos1, gos2, term_to_ancestors, term_to_freq, tot_freq)
                print('{} {} {} {} {} {}'.format(inner, acc1, acc2, sim22elem, gos1, gos2))
...
display_sim22s_selfexc(inner_to_outers, acc_to_gos, term_to_ancestors, go_to_freq, n_gos)
