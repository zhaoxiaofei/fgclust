...
\author[1]{Shing Hei Zhan\thanks{szhan@fusiongenomics.com ; TODO:PERSONAL-EMAIL-ADDRESS}}

\affil[1]{Fusion Genomics Inc.}
...
the PDB structures deposited exclusively before January 2\textsuperscript{nd} 2017, and Pfam-A-seed 31.0.
...
More specifically, 
	...
	and then aligns each input sequence to only a constant number of centre sequences \citep{steinegger2017linclust}.
Thus, the worst-case runtime of Linclust scales linearly with the number of input sequences.
...
Therefore, such premature determination of centroid can result in more clusters of worse quality.
...
Because deletions are ignored, sequence identity can overestimate the biological similarity between two sequences.
...
Moreover, edit-similarity is more correlated with biological similarity (here, measured in terms of three-dimensional protein structure and biological function) than sequence identity, as shown in \cref{table:pdb,fig:pfam,fig:rfam}.
...
To achieve linear expected runtime without significantly sacrificing sensitivity, FgClust uses longer long words for larger input size (i.e, the number of input sequences), and ignores long words that are both abundant and uninformative.
A long word is uninformative if and only if two uniformly, randomly, and independently sampled sequences that share this long word do not usually share sufficiently high edit-similarity.
Thus, two sequences sharing only uninformative and abundant long words will be ignored in a sequence search, thereby improving runtime.
...
Finally, FgClust only searches up to a maximum number of attempts.
...
Each true-positive target candidate increases the number of remaining attempts, whereas each false-positive target candidate decreases the number of remaining attempts.
...
Biologically similar sequences are not supposed to differ by sequence length.
Thus, UniRef90 and Uniref50 databases, which were constructed using CD-HIT, required a minimum sequence length overlap of 80\% \citep{suzek2014uniref}.
...
We evaluated FgClust and compared its performance to five state-of-the-arts clustering methods on several gold-standard biological databases.
...
Finally, we partially excluded the metrics of canonical specificity (i.e., either the percentage of clusters that are not corrupted or average consistency of each cluster).
...
Instead, we measured specificity with the average biological similarity between each input sequence and the representative centroid that covers the input sequence.
...
Nonetheless, we included canonical specificity in \cref{sec:appendix:canonical-specificity}.
...
Sequences that fail to be recruited by the centroid remain as singletons.
...
FgClust generates the fewest number of sequences with low TM scores (i.e., less than 0.5) to their centroids (\cref{table:pdb}).
...
%We found that other algorithms produce about 50\% more clusters with intra-cluster TM scores of less than 0.5 than FgClust.
...
The better specificity and sensitivity of FgClust on PDB structures is attributed to the use of Murphy10 to reduce amino-acid alphabet and 25 residues to normalize edit-similarity, respectively (data not shown).
...
We assessed the quality of the clusters in terms of the number of corrupted clusters and the number of corrupted sequences contained by a cluster.
We consider a cluster to be corrupted if it contains at least two sequences that belong to two different Pfam families.
Also, we regard a sequence, or a member of a cluster, to be corrupted if it belongs to a Pfam family that the representative of the cluster does not belong to.
...
the number of corrupted clusters is not normalized against the input and is therefore not meaningful for measuring specificity.
However, the number of corrupted clusters can reveal the characteristics of different clustering algorithms and is therefore still included.
...
The specificity of FgClust on Rfam-seed also is improved by the use of 25 residues to normalized edit-similarity (data not shown).

\section{Conclusion}

We developed FgClust, a novel algorithm for clustering biological sequences.
FgClust is more sensitive, more specific, and faster than other state-of-the-arts clustering algorithms, except that Linclust is two times faster but also generates up to more than 130\% more clusters than FgClust.
We observed that greedy incremental update may not select the optimal centroid during each incremental update.
Thus, FgClust detects a sufficient number of sequence pairs such that, in each pair, the first can represent the second.
...
Consequently, FgClust is and still will be able to handle the large volume of sequences released by UniProt and/or by metagenomic studies.
...
\begin{table}[t]%[!htbp]
	...
	\begin{tabular}{l c c c c c c c c c c}
		\toprule
		Program & similarity & 
		\multicolumn{8}{c}{TM scores of centroids modeling covered sequences} \\
		& cutoff      & 0.2 &
		\(\le\) 0.3 & \(\le\) 0.4 & \(\le\) 0.5 
