...
\title[FgClust]{
	%FgClust, a fast and scalable algorithm for generating high-quality clusters of biological sequences
...
\\
\textbf{Availability and implementation:} FgClust is available under the MIT license at \href{https://github.com/zhaoxiaofei/fgclust}{https://github.com/zhaoxiaofei/fgclust}\\
...
\Cref{sec:related-works} describes previous works related to FgClust in detail.
...
\Cref{sec:methods} describes the algorithm of FgClust in more detail.
...
\Cref{table:pdb,fig:pfam} show that FgClust always achieves the best sensitivity-specificity trade-off. \Cref{fig:uniref} shows that FgClust achieves the best sensitivity-runtime trade-off except Linclust. FgClust is approximately 150\% more sensitive (but only three times slower) than Linclust (\cref{fig:uniref}). Just like Linclust, the runtime of FgClust scales linearly with the size of input database (\cref{fig:uniref}).
\Cref{sec:discussion} discusses the evaluation results in more detail. %explains how and why each algorithmic novelty of FgClust improves sensitivity, specificity, runtime, and/or scalability.
...
\end{algorithm}
\footnotetext{
	By default, \(E=10^{6-10\cdot C}, L=\frac{80}{100}\), \(T=1+\lfloor 10\cdot (1-C)\rfloor\), \(W=100\cdot(10+E)\), and \(A=\frac{1}{20}\).
}
